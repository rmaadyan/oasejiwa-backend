...
export class PaymentService {
   ...
   * - Kirim Email Pengajuan Booking Baru (Menunggu Validasi Admin)
   */
  async payDP(userId: string, dto: ProcessPaymentDto, file: Express.Multer.File) {
    ...
    // Kirim notifikasi email setelah bukti pembayaran sukses diunggah
    ...
    this.emailService
      .sendNewBookingEmails({
        ...
        serviceName: booking.service?.nama || 'Konsultasi Psikologi',
      ...
      .catch((err) =>
        console.error('Gagal mengirim email pengajuan booking:', err),
      );
  ...
  async payFull(
    ...
    if (!fullPayment) {
      throw new BadRequestException(
        'Data pembayaran pelunasan tidak ditemukan',
      );
    }

    if (fullPayment.expiredAt < new Date()) {
      throw new BadRequestException(
        'Waktu pembayaran pelunasan sudah expired.',
      );
    }
    ...
    return {
      message:
        'Pembayaran pelunasan berhasil. Booking telah dikonfirmasi.',
      data: {
        bookingId: bookingId,
        bookingCode: booking.bookingCode,
