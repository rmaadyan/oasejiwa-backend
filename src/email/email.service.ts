...
export class EmailService {
  ...
  private getButton(label: string, url: string): string {
    return `
      <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="margin:12px 0 16px;">
        <tr>
          <td align="center">
  ...
  private getBaseEmailTemplate(
    ...
    <body style="margin:0;padding:0;background-color:${THEME.bgPage};font-family:${THEME.font};">
  ...
  async sendPasswordResetEmail(email: string, token: string) {
    const resetLink = `${process.env.FRONTEND_URL || 'https://oasejiwa.id'}/auth/reset-password?token=${token}`;
    const bodyContent = `
  ...
  async sendVerificationEmail(email: string, token: string) {
    const verifyLink = `${process.env.FRONTEND_URL || 'https://oasejiwa.id'}/auth/verify-email?token=${token}`;
    const bodyContent = `
      ...
      <p style="color:${THEME.body};font-size:12px;line-height:1.6;margin:0 0 18px;">
        Terima kasih telah mendaftar di <strong>Oase Jiwa</strong>. Tinggal satu langkah lagi untuk mengaktifkan akun Anda.
      </p>
      ${this.getButton('Verifikasi Email Saya', verifyLink)}
      ${this.getInfoBox('info', `Tautan verifikasi ini berlaku selama <strong>24 jam</strong>.`)}
  ...
  // 3. Kredensial Psikolog Baru
  async sendPsychologistCredentials(email: string, fullName: string, tempPassword: string) {
    const loginLink = `${process.env.FRONTEND_URL || 'https://oasejiwa.id'}/auth/signin`;
    const bodyContent = `
      ...
      <p style="color:${THEME.body};font-size:12px;line-height:1.6;margin:0 0 14px;">
        Akun Anda telah resmi dibuatkan sebagai <strong>Psikolog Praktisi di Oase Jiwa</strong>.
      </p>
      <table width="100%" cellpadding="0" cellspacing="0" role="presentation"
        ...
        <tr>
          <td>
            <p style="color:${THEME.muted};font-size:10.5px;margin:0 0 3px;text-transform:uppercase;">Email Login</p>
            <p style="color:${THEME.ink};font-size:12px;font-weight:600;margin:0 0 10px;">${email}</p>
            <p style="color:${THEME.muted};font-size:10.5px;margin:0 0 3px;text-transform:uppercase;">Password Sementara</p>
            <span style="font-family:monospace;font-size:14px;font-weight:700;color:${THEME.navy};
      ...
      ${this.getInfoBox('warning', `<strong>⚠️ Wajib Update Profil:</strong> Harap login dan lengkapi data profil serta jadwal praktik Anda.`)}
    ...
    try {
      ...
      this.logger.error(`Failed to send credentials to ${email}`, error.stack);
    }
  }

  // 4. Reminder Profil Psikolog
  async sendPsychologistReminderEmail(email: string, fullName: string) {
    ...
    const bodyContent = `
      ...
      <p style="color:${THEME.body};font-size:12px;line-height:1.6;margin:0 0 18px;">
        Akun psikolog Anda masih menunggu kelengkapan profil. Mohon segera login untuk melengkapi biodata profesional Anda.
      </p>
  ...
  // 5. Booking Baru (User: My Booking, Psych: Jadwal Saya)
  async sendNewBookingEmails(data: BookingEmailData) {
    ...
    const userBody = `
      ...
      <p style="color:${THEME.body};font-size:12.5px;line-height:1.6;margin:0 0 16px;">
        Halo <strong>${data.userName}</strong>, pengajuan booking Anda telah kami terima dan saat ini <strong>sedang menunggu verifikasi pembayaran oleh tim Admin</strong>.
      </p>
      ...
      ${this.getInfoBox(
        ...
        1. Tim Admin akan memvalidasi bukti pembayaran DP Anda dalam waktu <strong>1x24 jam</strong>.<br/>
        2. Setelah divalidasi, Anda akan menerima email konfirmasi resmi beserta bukti invoice.<br/>
        3. Pantau status pemesanan Anda melalui menu <strong>Riwayat Booking</strong> di profil akun.`,
      )}

      ${this.getButton('Lihat Riwayat Booking Saya', userMyBookingUrl)}
      ...
      <div style="background-color:${THEME.bgSubtle};border:1px solid ${THEME.border};border-radius:10px;padding:14px;text-align:center;margin-top:16px;">
        <p style="color:${THEME.muted};font-size:11px;margin:0 0 4px;">
          Butuh konfirmasi lebih cepat? Hubungi Customer Support:
        </p>
    ...
    const psychBody = `
      ...
      ${this.getInfoBox('info', 'Jadwal ini akan resmi dikonfirmasi setelah bukti pembayaran divalidasi oleh Administrator.')}

      ${this.getButton('Buka Dashboard Jadwal Saya', psychScheduleUrl)}
    `;
  ...
  // 6. Booking Approval (User: My Booking, Psych: Jadwal Saya)
  async sendBookingApprovalEmail(data: BookingEmailData) {
    ...
    const userBody = `
      <div style="background-color:${THEME.bgCard};border:1px solid ${THEME.border};border-radius:12px;padding:22px 20px;margin-bottom:16px;">
        <div style="text-align:center;padding-bottom:16px;border-bottom:1px dashed ${THEME.border};margin-bottom:16px;">
          ...
          <h1 style="color:${THEME.ink};font-size:26px;font-weight:800;margin:0 0 12px;">${formattedDp}</h1>
      ...
      <div style="background-color:${THEME.bgSubtle};border:1px solid ${THEME.border};border-radius:10px;padding:14px;text-align:center;">
        <p style="color:${THEME.muted};font-size:11px;margin:0 0 4px;">
          Untuk informasi lebih lanjut mengenai sesi Anda, hubungi Admin:
        </p>
        ${this.getWhatsAppButton(data.bookingCode)}
      </div>
    `;
    ...
    const psychBody = `
      ...
      <p style="color:${THEME.body};font-size:12.5px;line-height:1.6;margin:0 0 16px;">
        Halo <strong>${data.psychologistName}</strong>, pembayaran DP pasien telah divalidasi. Jadwal konsultasi berikut telah <strong>resmi dikonfirmasi</strong>:
      </p>
  ...
  // 8. Reschedule Jadwal (Kirim ke User & Psikolog)
  async sendRescheduleEmails(data: BookingEmailData) {
    ...
    const psychBody = `
      <h2 style="color:${THEME.ink};font-size:17px;font-weight:700;margin:0 0 10px;">Pemberitahuan Perubahan Jadwal Sesi 🔄</h2>
      <p style="color:${THEME.body};font-size:12.5px;line-height:1.6;margin:0 0 16px;">
        Halo <strong>${data.psychologistName}</strong>, jadwal konsultasi pasien berikut telah diubah:
      </p>
      ...
      ${this.getInfoCard(
        [
          ...
          ['Layanan', data.serviceName || 'Konseling'],
          ['Jadwal Baru', `<span style="color:#2563eb;font-weight:bold;">${data.newScheduledDate} (${data.newScheduledTime} WIB)</span>`],
          ['Alasan', data.reason || 'Penyesuaian jadwal'],
        ],
        '#2563eb',
      )}
    ...
    try {
      ...
      await Promise.all(sends);
      this.logger.log(`Reschedule emails sent to user & psychologist for #${data.bookingCode}`);
    } catch (error: any) {
